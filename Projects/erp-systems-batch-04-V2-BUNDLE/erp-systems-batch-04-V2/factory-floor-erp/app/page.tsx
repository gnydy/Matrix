      ...
      <Topbar />
        ...
        <section className="px-5 py-8 lg:px-8">
          <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
            <div className="card rounded-[2rem] p-6"><p className="text-xs font-black uppercase tracking-[0.35em] text-slate-400">Factory floor board</p><h2 className="mt-5 text-5xl font-black leading-none">{site.name}</h2><p className="mt-5 text-slate-600">{site.concept}</p><div className="mt-8 space-y-3">{modules.map((m) => <div key={m} className="rounded-2xl bg-slate-100 p-4 font-bold">{m}</div>)}</div></div>
