export const site = {
  "name": "Campus Admin ERP",
  "slug": "campus-admin-erp",
  "industry": "Schools and universities",
  "concept": "Admissions, classes, student fees, teachers, attendance and academic operations",
  "layout": "Campus operations hub",
  "port": 3069,
  "accent": "#6366f1"
} as const;

export const modules = [
  "Admissions",
  "Classes",
  "Fees",
  "Academic alerts"
] as const;
export const records = [
  {
    "id": "ERP-1001",
    "name": "Admissions",
    "owner": "Operations Lead",
    "status": "Healthy",
    "value": "98%",
    "trend": "+12%"
  },
  {
    "id": "ERP-1002",
    "name": "Classes",
    "owner": "Finance Manager",
    "status": "Review",
    "value": "74%",
    "trend": "-3%"
  },
  {
    "id": "ERP-1003",
    "name": "Fees",
    "owner": "Supply Chain",
    "status": "Delayed",
    "value": "21",
    "trend": "5 risks"
  },
  {
    "id": "ERP-1004",
    "name": "Academic alerts",
    "owner": "Admin",
    "status": "Approved",
    "value": "$248k",
    "trend": "+8%"
  }
] as const;
export const phases = [
  "Request",
  "Review",
  "Approval",
  "Execution",
  "Audit"
] as const;
export const stats = [
  {
    "label": "Active modules",
    "value": "12",
    "detail": "Role-based screens"
  },
  {
    "label": "Open approvals",
    "value": "15",
    "detail": "Needs manager review"
  },
  {
    "label": "Monthly volume",
    "value": "$180k",
    "detail": "Mock operational value"
  },
  {
    "label": "Automation rate",
    "value": "70%",
    "detail": "Template readiness score"
  }
] as const;

export const navItems = ['Overview', 'Modules', 'Records', 'Approvals', 'Reports', 'Settings'] as const;
