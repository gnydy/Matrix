      ...
      <Topbar />
        ...
        <section className="px-5 py-8 lg:px-8">
          <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="card rounded-[2rem] p-8"><p className="text-xs font-black uppercase tracking-[0.35em] text-slate-400">Kitchen and supply desk</p><h2 className="mt-4 text-5xl font-black">{site.concept}</h2><div className="mt-8 grid gap-3 sm:grid-cols-2">{modules.map((m, i) => <div key={m} className="rounded-2xl border border-slate-200 bg-white p-4"><b>{m}</b><p className="mt-2 text-sm text-slate-500">Queue #{i+1} with filters and actions.</p></div>)}</div></div>
