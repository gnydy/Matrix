    ...
    <Shell>
      <Topbar />
        ...
        <section className="grid min-h-[720px] gap-0 lg:grid-cols-[420px_1fr]">
          <div className="bg-slate-950 p-8 text-white"><p className="text-xs font-black uppercase tracking-[0.35em] text-white/40">Portfolio occupancy map</p><h2 className="mt-5 text-5xl font-black leading-tight">{site.concept}</h2><div className="mt-8 space-y-3">{records.map((r) => <div key={r.id} className="rounded-3xl bg-white/10 p-4"><div className="flex justify-between"><b>{r.name}</b><span>{r.value}</span></div><p className="mt-1 text-sm text-white/50">{r.trend}</p></div>)}</div></div>
