      ...
      <Topbar />
        ...
        <section className="px-5 py-8 lg:px-8">
          <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
            <div className="card rounded-[2rem] p-8"><p className="text-xs font-black uppercase tracking-[0.35em] text-slate-400">Field service command</p><h2 className="mt-5 text-5xl font-black">{site.concept}</h2><div className="mt-8 grid gap-4 md:grid-cols-2">{modules.map((m, i) => <div key={m} className="rounded-3xl border border-slate-200 p-5"><div className="text-4xl font-black text-[var(--accent)]">{i+1}</div><h3 className="mt-4 text-xl font-black">{m}</h3><p className="mt-2 text-sm text-slate-500">Profile-driven workflow with notes, states and ownership.</p></div>)}</div></div>
