...
export function Badge({ children }: { children: React.ReactNode }) {
  return <span className="inline-flex rounded-full border border-cyan-300/40 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300">{children}</span>;
}
...
export function Header({ mode = 'top' }: { mode?: 'top' | 'side' | 'split' | 'compact' }) {
  if (mode === 'side') {
    return <aside className="fixed left-0 top-0 z-20 hidden h-screen w-64 border-r border-white/10 bg-white/5 text-white p-6 lg:block">
      ...
      <Link href="/checkout" className="mt-10 block rounded-2xl bg-gradient-to-r from-cyan-400 to-blue-600 px-5 py-3 text-center text-sm font-black text-white">Checkout Mock</Link>
    </aside>;
  }
  return <header className="sticky top-0 z-30 border-b border-white/10 bg-white/5 text-white/80 px-5 py-4 backdrop-blur-xl">
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2">
        <Link href="/account" className="rounded-full border border-cyan-300/40 px-4 py-2 text-sm font-bold">Account</Link>
        <Link href="/cart" className="rounded-full bg-gradient-to-r from-cyan-400 to-blue-600 px-4 py-2 text-sm font-black text-white">Cart</Link>
      </div>
    </div>
  </header>;
}
...
export function ProductCard({ product, wide = false }: { product: typeof products[number], wide?: boolean }) {
  return <Link href={`/products/${product.slug}`} className={`group block overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 text-white ${wide ? 'md:grid md:grid-cols-[1fr_1.2fr]' : ''}`}>
    <div className="relative min-h-56 overflow-hidden bg-gradient-to-br from-cyan-400 to-blue-600">
    ...
    <div className="p-6">
      <p className="text-xs font-black uppercase tracking-[0.25em] text-cyan-300">{product.category}</p>
      ...
      <div className="mt-5 flex items-center justify-between">
        ...
        <span className="rounded-full border border-cyan-300/40 px-4 py-2 text-xs font-black">View</span>
      </div>
    </div>
  </Link>;
}
