export const site = {
  name: 'GreenFleet Carbon Logistics',
  slug: 'greenfleet-carbon-logistics',
  category: 'Logistics & Delivery Systems',
  industry: 'Green Logistics',
  client: 'Sustainable logistics provider',
  audience: 'Sustainable delivery companies, EV fleets, ESG-focused logistics teams',
  positioning: 'A green logistics dashboard for route efficiency, carbon estimates, EV fleet status, and client sustainability reports.',
  visual: 'Sustainability dashboard with carbon cards, EV status, route efficiency, and client report previews',
  complexity: 'Medium',
  hero: 'Show cleaner delivery performance with carbon metrics, EV fleet visibility, and route efficiency.',
  cta: 'View Carbon Report',
  accent: '#22c55e',
  dark: '#14532d',
  metrics: [
  {
    "label": "Active shipments",
    "value": "4,306",
    "note": "+12.4%"
  },
  {
    "label": "On-time rate",
    "value": "95.0%",
    "note": "SLA protected"
  },
  {
    "label": "Open exceptions",
    "value": "36",
    "note": "Needs review"
  },
  {
    "label": "Fleet utilization",
    "value": "86%",
    "note": "Capacity live"
  }
],
  features: [
  "Live shipment timeline with delivery milestones and exception states",
  "Driver and vehicle visibility with utilization and status scoring",
  "Route planning board with SLA risk, stops, ETA, and operating capacity",
  "Customer-facing tracking experience ready to connect with a backend later",
  "Operations dashboard designed for managers, dispatchers, and support teams",
  "Mock data architecture that can be replaced with API responses quickly"
],
  modules: [
  {
    "title": "Tracking Console",
    "description": "Search shipments, view ETA, status, route movement, and exception history."
  },
  {
    "title": "Dispatch Board",
    "description": "Assign drivers, rebalance routes, and recover delayed orders from one board."
  },
  {
    "title": "Fleet & Drivers",
    "description": "Monitor vehicle readiness, driver status, compliance, and performance signals."
  },
  {
    "title": "Customer Updates",
    "description": "Prepare notifications, delivery preferences, and support escalation flows."
  }
],
  orders: [
  {
    "id": "LG-18001",
    "customer": "Northstar Retail",
    "route": "Cairo → Giza",
    "status": "Out for delivery",
    "eta": "09:30",
    "risk": "Medium"
  },
  {
    "id": "LG-18002",
    "customer": "BlueCart Egypt",
    "route": "Alexandria → Cairo",
    "status": "Delayed",
    "eta": "11:15",
    "risk": "High"
  },
  {
    "id": "LG-18003",
    "customer": "MedPlus Distribution",
    "route": "6 October → Nasr City",
    "status": "Delivered",
    "eta": "13:45",
    "risk": "Low"
  },
  {
    "id": "LG-18004",
    "customer": "Urban Home",
    "route": "Port Said → Cairo",
    "status": "Awaiting pickup",
    "eta": "16:20",
    "risk": "Medium"
  },
  {
    "id": "LG-18005",
    "customer": "Prime Foods",
    "route": "Maadi → New Cairo",
    "status": "Customs review",
    "eta": "18:10",
    "risk": "Low"
  },
  {
    "id": "LG-18006",
    "customer": "Atlas Components",
    "route": "Airport Hub → Downtown",
    "status": "In transit",
    "eta": "21:00",
    "risk": "Low"
  }
],
  drivers: [
  {
    "name": "Omar Adel",
    "vehicle": "Van 204",
    "zone": "West",
    "score": "86%",
    "status": "Exception"
  },
  {
    "name": "Karim Nabil",
    "vehicle": "Bike 88",
    "zone": "East",
    "score": "87%",
    "status": "Available"
  },
  {
    "name": "Mina Sameh",
    "vehicle": "Truck 17",
    "zone": "Airport",
    "score": "88%",
    "status": "On route"
  },
  {
    "name": "Youssef Ali",
    "vehicle": "EV 11",
    "zone": "Downtown",
    "score": "89%",
    "status": "Break"
  },
  {
    "name": "Nour Hassan",
    "vehicle": "Van 322",
    "zone": "New Cairo",
    "score": "90%",
    "status": "Loading"
  }
],
  routes: [
  {
    "name": "Priority SLA lane",
    "stops": 17,
    "distance": "140 km",
    "health": "Optimized"
  },
  {
    "name": "Dense urban cluster",
    "stops": 32,
    "distance": "60 km",
    "health": "Traffic risk"
  },
  {
    "name": "Regional transfer",
    "stops": 10,
    "distance": "328 km",
    "health": "On schedule"
  },
  {
    "name": "Recovery route",
    "stops": 11,
    "distance": "97 km",
    "health": "Needs dispatch"
  }
],
  timeline: [
    { step: 'Order received', detail: 'Customer request created and validated', state: 'Done' },
    { step: 'Hub scan', detail: 'Package scanned at operational hub', state: 'Done' },
    { step: 'Driver assigned', detail: 'Best route and driver selected', state: 'Active' },
    { step: 'Delivery proof', detail: 'Signature, image, OTP, or customer confirmation', state: 'Pending' }
  ]
};

export type Site = typeof site;
