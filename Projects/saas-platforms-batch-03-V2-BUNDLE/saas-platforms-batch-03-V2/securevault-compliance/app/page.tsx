  ...
  return (
    <main className="min-h-screen bg-black text-white">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 grid max-w-7xl gap-8 md:grid-cols-[0.9fr_1.1fr]"><div><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-xl text-slate-300">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Audit dashboard</SecondaryLink></div></div><div className="rounded-[2rem] border border-emerald-400/20 bg-emerald-400/5 p-6 font-mono text-sm"><p className="text-emerald-300">$ audit run --controls SOC2 ISO27001 GDPR</p><div className="mt-6 space-y-3">{highlights.map(h => <div key={h} className="rounded-xl border border-emerald-400/20 bg-black p-4 text-emerald-100">✓ {h} verified</div>)}</div></div></div>
      </section>
