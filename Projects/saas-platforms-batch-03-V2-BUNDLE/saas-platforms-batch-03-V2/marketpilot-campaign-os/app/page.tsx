  ...
  return (
    <main className="min-h-screen bg-fuchsia-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 max-w-7xl"><Badge>{template.archetype}</Badge><h1 className="mt-6 max-w-4xl text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 max-w-3xl text-xl text-slate-600">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Campaign board</SecondaryLink></div><div className="mt-12 grid gap-3 md:grid-cols-7">{Array.from({length:21}).map((_,i)=> <div key={i} className={`min-h-28 rounded-2xl border p-3 ${i%5===0?'bg-slate-950 text-white':'bg-white'}`}><span className="text-sm">Day {i+1}</span>{i%5===0 && <p className="mt-4 text-sm">Launch asset</p>}</div>)}</div></div>
      </section>
