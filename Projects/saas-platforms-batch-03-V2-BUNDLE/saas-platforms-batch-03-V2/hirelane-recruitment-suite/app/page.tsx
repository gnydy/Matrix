    ...
    <main className="min-h-screen bg-rose-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 max-w-7xl">
          <div className="max-w-4xl"><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-xl text-slate-600">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Pipeline demo</SecondaryLink></div></div>
          <div className="mt-12 grid gap-4 md:grid-cols-4">{["Sourced", "Screening", "Interview", "Offer"].map((lane, i) => <div key={lane} className="rounded-3xl bg-white p-4 shadow-sm"><h3 className="font-semibold">{lane}</h3><div className="mt-4 space-y-3">{activity.slice(0,3).map((a, idx) => <div key={a.title+lane} className="rounded-2xl border border-rose-100 bg-rose-50 p-4 text-sm">Candidate {i+idx+1}<br/><span className="text-slate-500">{a.status}</span></div>)}</div></div>)}</div>
        </div>
      </section>
