  ...
  return (
    <main className="min-h-screen bg-green-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 grid max-w-7xl gap-8 md:grid-cols-[1fr_520px]"><div><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-xl text-slate-600">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Payments dashboard</SecondaryLink></div></div><div className="space-y-4">{metrics.map((m,i) => <div key={m.label} className={`rounded-[2rem] p-6 ${i===0?'bg-slate-950 text-white':'bg-white'} shadow-sm`}><p className="text-sm opacity-70">{m.label}</p><div className="mt-8 text-4xl font-semibold">{m.value}</div></div>)}</div></div>
      </section>
