  ...
  return (
    <main className="min-h-screen bg-white">
      <section className="px-6 py-8">
        ...
        <div className="mx-auto mt-10 max-w-7xl rounded-[3rem] bg-slate-950 p-6 text-white md:p-10">
          <div className="grid gap-8 md:grid-cols-[1fr_420px]">
            <div className="rounded-[2.5rem] bg-white p-8 text-slate-950 md:p-12">
              <Badge>{template.archetype}</Badge>
              <h1 className="mt-8 text-5xl font-semibold tracking-tight md:text-7xl">{template.name}</h1>
              <p className="mt-6 text-xl leading-8 text-slate-600">{template.hero}</p>
              <div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/pricing">Compare plans</SecondaryLink></div>
            </div>
            <div className="space-y-4">{metrics.map((m) => <MetricCard key={m.label} value={m.value} label={m.label} />)}</div>
          </div>
          <div className="mt-6 grid gap-4 md:grid-cols-4">{highlights.map((h, i) => <div key={h} className="rounded-3xl border border-white/10 bg-white/10 p-5"><p className="text-sm text-slate-300">0{i+1}</p><h3 className="mt-8 font-semibold">{h}</h3></div>)}</div>
        </div>
      </section>
