        ...
        <div className="mx-auto mt-12 max-w-7xl">
          <div className="grid gap-8 md:grid-cols-[0.8fr_1.2fr]">
            <div className="rounded-[2rem] bg-slate-950 p-8 text-white md:p-12">
              <Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-lg leading-8 text-emerald-50/80">{template.hero}</p><div className="mt-8 flex flex-wrap gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Ledger preview</SecondaryLink></div>
            </div>
            <div className="rounded-[2rem] border border-emerald-200 bg-white p-6 shadow-soft">
              <div className="flex items-center justify-between border-b border-slate-200 pb-5"><strong>Billing ledger</strong><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs text-emerald-700">Reconciled</span></div>
              <div className="mt-6 space-y-3">{rows.map((r) => <div key={r.name} className="grid grid-cols-4 items-center rounded-2xl bg-slate-50 p-4 text-sm"><span className="font-medium">{r.name}</span><span>{r.owner}</span><span>{r.state}</span><span className="text-right font-semibold">{r.score}</span></div>)}</div>
              <div className="mt-6 grid gap-4 md:grid-cols-3">{metrics.map((m) => <div key={m.label} className="rounded-2xl bg-emerald-950 p-5 text-white"><b className="text-2xl">{m.value}</b><p className="text-sm text-emerald-100">{m.label}</p></div>)}</div>
            </div>
          </div>
        </div>
      </section>
