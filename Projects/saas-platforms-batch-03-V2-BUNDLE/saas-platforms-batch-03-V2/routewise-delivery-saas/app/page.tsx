  ...
  return (
    <main className="min-h-screen bg-lime-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 grid max-w-7xl gap-8 md:grid-cols-[1fr_520px]"><div className="rounded-[2rem] bg-slate-950 p-8 text-white"><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold">{template.name}</h1><p className="mt-6 text-slate-300">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Dispatch board</SecondaryLink></div></div><div className="relative min-h-[520px] rounded-[2rem] bg-white p-6 shadow-soft"><div className="absolute inset-8 rounded-[2rem] bg-[radial-gradient(circle_at_20%_30%,#84cc16_0_4%,transparent_5%),radial-gradient(circle_at_80%_60%,#0f172a_0_3%,transparent_4%),radial-gradient(circle_at_45%_75%,#22c55e_0_4%,transparent_5%)]"/><div className="relative z-10 space-y-3">{activity.map(a => <div key={a.title} className="w-72 rounded-2xl bg-white/90 p-4 shadow"><b>{a.title}</b><p className="text-sm text-slate-500">{a.status}</p></div>)}</div></div></div>
      </section>
