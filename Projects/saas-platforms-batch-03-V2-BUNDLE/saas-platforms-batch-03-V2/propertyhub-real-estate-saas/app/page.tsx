  ...
  return (
    <main className="min-h-screen bg-stone-100">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 max-w-7xl"><div className="grid gap-8 md:grid-cols-[1fr_1.2fr]"><div><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-xl text-slate-600">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Listings dashboard</SecondaryLink></div></div><div className="grid gap-4 md:grid-cols-2">{[1,2,3,4].map(n => <div key={n} className="overflow-hidden rounded-[2rem] bg-white shadow-sm"><div className="h-36 bg-gradient-to-br from-stone-300 to-slate-800"/><div className="p-5"><b>Property Unit {n}</b><p className="text-sm text-slate-500">Viewing pipeline active</p></div></div>)}</div></div></div>
      </section>
