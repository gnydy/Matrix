...
export default function HomePage() {
  return (
    <main className="min-h-screen bg-slate-950">
      <section className="grid min-h-screen md:grid-cols-[92px_1fr]">
        <aside className="hidden border-r border-white/10 bg-black/20 p-4 text-white md:block">
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-slate-950 font-black">OP</div>
          <div className="mt-10 space-y-4 text-xs text-slate-400 [writing-mode:vertical-rl]">{template.nav}</div>
        </aside>
        <div className="px-6 py-8 md:px-12">
          <MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
          <div className="mx-auto grid max-w-7xl gap-10 py-16 md:grid-cols-[0.9fr_1.1fr] md:items-center">
            <div>
              <Badge>{template.archetype}</Badge>
              <h1 className="mt-6 text-5xl font-semibold tracking-tight text-white md:text-7xl">{template.name}</h1>
              <p className="mt-6 max-w-2xl text-xl leading-8 text-slate-300">{template.hero}</p>
              <div className="mt-8 flex flex-wrap gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">See control room</SecondaryLink></div>
              <div className="mt-10 grid gap-4 md:grid-cols-3">{metrics.map((m) => <MetricCard key={m.label} value={m.value} label={m.label} />)}</div>
            </div>
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl">
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-3xl bg-indigo-400/20 p-5 text-white md:row-span-2"><p className="text-sm text-indigo-100">Workflow map</p><div className="mt-8 space-y-3">{highlights.map((h) => <div key={h} className="rounded-2xl bg-white/10 p-4">{h}</div>)}</div></div>
                <div className="rounded-3xl bg-white p-5 text-slate-950 md:col-span-2"><p className="font-semibold">Approval lane</p><div className="mt-4 grid gap-3">{activity.slice(0,3).map((a) => <div key={a.title} className="rounded-xl bg-slate-100 p-4 text-sm">{a.title}</div>)}</div></div>
                <div className="rounded-3xl border border-white/10 p-5 text-white md:col-span-2"><p className="text-sm text-slate-300">Automation ROI</p><div className="mt-8 h-28 rounded-2xl bg-gradient-to-r from-indigo-500 via-cyan-400 to-emerald-300" /></div>
              </div>
            </div>
          </div>
        </div>
