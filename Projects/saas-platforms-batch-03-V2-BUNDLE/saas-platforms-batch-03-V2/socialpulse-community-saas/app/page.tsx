  ...
  return (
    <main className="min-h-screen bg-red-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 grid max-w-7xl gap-8 md:grid-cols-[420px_1fr]"><div><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold">{template.name}</h1><p className="mt-6 text-lg text-slate-600">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Community dashboard</SecondaryLink></div></div><div className="grid gap-4 md:grid-cols-[1fr_280px]"><div className="space-y-4">{activity.map(a => <article key={a.title} className="rounded-[2rem] bg-white p-6 shadow-sm"><b>{a.title}</b><p className="mt-2 text-slate-600">Member discussion, reaction controls, moderation status, and engagement quality.</p></article>)}</div><aside className="rounded-[2rem] bg-slate-950 p-6 text-white">{metrics.map(m => <div key={m.label} className="border-b border-white/10 py-5"><b className="text-3xl">{m.value}</b><p className="text-sm text-slate-300">{m.label}</p></div>)}</aside></div></div>
      </section>
