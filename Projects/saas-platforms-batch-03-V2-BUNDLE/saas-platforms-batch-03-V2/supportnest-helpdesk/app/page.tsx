  ...
  return (
    <main className="min-h-screen bg-amber-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-10 grid max-w-7xl gap-8 md:grid-cols-[360px_1fr]">
          <aside className="rounded-[2rem] bg-slate-950 p-6 text-white"><Badge>{template.archetype}</Badge><h1 className="mt-6 text-4xl font-semibold">{template.name}</h1><p className="mt-4 text-slate-300">{template.hero}</p><div className="mt-8 space-y-3">{metrics.map(m => <MetricCard key={m.label} value={m.value} label={m.label} />)}</div></aside>
          <div className="rounded-[2rem] bg-white p-5 shadow-soft"><div className="grid gap-4 md:grid-cols-[300px_1fr]"><div className="space-y-3">{activity.map(a => <div key={a.title} className="rounded-2xl border border-slate-200 p-4"><b>{a.title}</b><p className="text-sm text-slate-500">{a.status} · {a.time}</p></div>)}</div><div className="rounded-3xl bg-slate-100 p-6"><h2 className="text-3xl font-semibold">Customer conversation</h2><p className="mt-4 text-slate-600">Suggested macro replies, SLA state, escalation owner, and knowledge base recommendations are visible in one desk.</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Inbox dashboard</SecondaryLink></div></div></div></div>
        </div>
      </section>
