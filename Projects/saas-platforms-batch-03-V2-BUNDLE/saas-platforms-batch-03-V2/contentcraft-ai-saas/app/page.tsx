  ...
  return (
    <main className="min-h-screen bg-purple-950 text-white">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 grid max-w-7xl gap-8 md:grid-cols-[0.9fr_1.1fr]"><div><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-xl text-purple-100/80">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">AI history</SecondaryLink></div></div><div className="rounded-[2rem] border border-white/10 bg-white/10 p-5"><div className="rounded-3xl bg-black/30 p-5"><p className="text-sm text-purple-200">Prompt box</p><div className="mt-4 rounded-2xl bg-white p-5 text-slate-950">Write a launch campaign for a B2B SaaS product...</div><div className="mt-4 grid gap-3 md:grid-cols-2">{highlights.map(h => <div key={h} className="rounded-2xl bg-white/10 p-4">{h}</div>)}</div></div></div></div>
      </section>
