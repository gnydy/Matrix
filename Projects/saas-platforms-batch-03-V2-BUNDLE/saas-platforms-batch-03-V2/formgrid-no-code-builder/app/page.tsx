  ...
  return (
    <main className="min-h-screen bg-pink-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 grid max-w-7xl gap-8 md:grid-cols-[300px_1fr_320px]"><aside className="rounded-[2rem] bg-white p-5 shadow-sm"><b>Blocks</b>{highlights.map(h => <div key={h} className="mt-3 rounded-xl bg-pink-100 p-3 text-sm">{h}</div>)}</aside><div className="rounded-[2rem] bg-white p-6 shadow-soft"><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold">{template.name}</h1><p className="mt-6 text-lg text-slate-600">{template.hero}</p><div className="mt-10 rounded-3xl border-2 border-dashed border-pink-300 p-8 text-center">Drag workflow components here</div><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Builder dashboard</SecondaryLink></div></div><aside className="rounded-[2rem] bg-slate-950 p-5 text-white"><b>Properties</b>{metrics.map(m => <div key={m.label} className="mt-4 rounded-xl bg-white/10 p-4"><b>{m.value}</b><p className="text-sm text-slate-300">{m.label}</p></div>)}</aside></div>
      </section>
