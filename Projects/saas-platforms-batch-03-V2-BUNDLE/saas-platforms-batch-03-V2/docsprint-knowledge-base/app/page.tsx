  ...
  return (
    <main className="min-h-screen bg-slate-100">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 overflow-hidden rounded-[2rem] bg-white shadow-soft"><div className="grid min-h-[620px] md:grid-cols-[260px_1fr_300px]"><aside className="border-r border-slate-200 p-6"><b>{template.name}</b><div className="mt-8 space-y-3">{highlights.map(h=> <p key={h} className="rounded-xl bg-slate-100 p-3 text-sm">{h}</p>)}</div></aside><article className="p-8 md:p-12"><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold">{template.name}</h1><p className="mt-6 text-xl text-slate-600">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Docs analytics</SecondaryLink></div></article><aside className="border-l border-slate-200 p-6"><p className="font-semibold">On this page</p>{metrics.map(m => <div key={m.label} className="mt-4 rounded-xl bg-slate-50 p-4"><b>{m.value}</b><p className="text-sm text-slate-500">{m.label}</p></div>)}</aside></div></div>
      </section>
