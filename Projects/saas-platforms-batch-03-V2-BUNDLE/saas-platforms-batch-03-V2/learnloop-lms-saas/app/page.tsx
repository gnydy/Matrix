  ...
  return (
    <main className="min-h-screen bg-violet-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 max-w-7xl"><div className="grid gap-8 md:grid-cols-[1fr_520px]"><div><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-xl text-slate-600">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Student dashboard</SecondaryLink></div></div><div className="grid grid-cols-2 gap-4">{highlights.map((h,i)=> <div key={h} className={`rounded-[2rem] p-6 ${i===0?'bg-slate-950 text-white':'bg-white text-slate-950'} shadow-sm`}><div className="text-5xl font-semibold">{i*18+24}%</div><p className="mt-6 font-medium">{h}</p></div>)}</div></div></div>
      </section>
