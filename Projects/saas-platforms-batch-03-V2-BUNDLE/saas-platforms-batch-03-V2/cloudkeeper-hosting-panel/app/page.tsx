  ...
  return (
    <main className="min-h-screen bg-blue-950 text-white">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 grid max-w-7xl gap-8 md:grid-cols-[0.9fr_1.1fr]"><div><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-xl text-blue-100/80">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">Server panel</SecondaryLink></div></div><div className="grid gap-3">{Array.from({length:8}).map((_,i)=> <div key={i} className="rounded-2xl border border-blue-300/20 bg-white/10 p-4"><div className="flex items-center justify-between"><span>Node-{i+1}</span><span className="text-emerald-300">online</span></div></div>)}</div></div>
      </section>
