  ...
  return (
    <main className="min-h-screen bg-orange-50">
      <section className="px-6 py-8"><MiniNav links={["Home", "Pricing", "Dashboard", "Auth"]} />
        <div className="mx-auto mt-12 max-w-7xl"><div className="grid gap-8 md:grid-cols-[1fr_1fr]"><div><Badge>{template.archetype}</Badge><h1 className="mt-6 text-5xl font-semibold md:text-7xl">{template.name}</h1><p className="mt-6 text-xl text-slate-600">{template.hero}</p><div className="mt-8 flex gap-3"><PrimaryLink>{template.cta}</PrimaryLink><SecondaryLink href="/dashboard">CRM board</SecondaryLink></div></div><div className="rounded-[2rem] bg-white p-5 shadow-soft"><div className="grid grid-cols-3 gap-3">{["Lead", "Proposal", "Won"].map(stage => <div key={stage}><b>{stage}</b><div className="mt-3 space-y-3">{rows.slice(0,3).map(r => <div key={r.name+stage} className="rounded-2xl bg-orange-50 p-4 text-sm">{r.name}<br/><span className="text-slate-500">{r.score}</span></div>)}</div></div>)}</div></div></div></div>
      </section>
