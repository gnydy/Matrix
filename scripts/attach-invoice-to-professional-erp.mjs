...
const nextConfig =
  readIf('next.config.js') +
  readIf('next.config.mjs') +
  readIf('next.config.ts');

const hasBasePath = nextConfig.includes('basePath') && nextConfig.includes('/erp');

console.log('basePath /erp:', hasBasePath);

// حذف المسارات التجريبية والمكررة
rm('src/app/invoices');

// لو عندك basePath /erp يبقى المسار الصحيح داخل (erp)
// لو لا يوجد basePath يبقى المسار الصحيح داخل app/erp
if (hasBasePath) {
...
// حذف layout المؤقت الذي عملناه لو كان موجودًا، بدون لمس layout الاحترافي الأصلي
const tempLayoutPath = path.join(web, 'src', 'app', 'erp', 'layout.tsx');
if (fs.existsSync(tempLayoutPath)) {
  const tempLayout = fs.readFileSync(tempLayoutPath, 'utf8');

  if (
    tempLayout.includes('لوحة تشغيل ERP') &&
    tempLayout.includes('Matrix ERP') &&
    tempLayout.includes('navItems')
  ) {
    fs.rmSync(tempLayoutPath, { force: true });
    console.log('removed temporary ERP layout');
  }
}
...
export function NewInvoiceProfessionalClient() {
  ...
  return (
    <div style={{ display: 'grid', gap: 20 }}>
      <section style={{
        ...
        <h1 style={{ fontSize: 34, margin: '0 0 8px', color: '#22c55e' }}>
          فاتورة جديدة
        </h1>

        <p style={{ margin: 0, color: '#94a3b8', fontSize: 18 }}>
          أضف المنتجات، عدّل الكمية والسعر، احسب الإجمالي، ثم سجل الفاتورة.
        </p>
...
const page = `
import { NewInvoiceProfessionalClient } from '@/components/invoices/new-invoice-professional-client';

export default function NewInvoicePage() {
  return <NewInvoiceProfessionalClient />;
}
`;

write('src/components/invoices/new-invoice-professional-client.tsx', client);

const route = hasBasePath
  ? 'src/app/(erp)/invoices/new/page.tsx'
  : 'src/app/erp/invoices/new/page.tsx';

write(route, page);

console.log('DONE: invoice attached to professional ERP');
console.log('ROUTE_FILE:', route);
console.log('OPEN:', hasBasePath ? 'http://localhost:3002/erp/invoices/new' : 'http://localhost:3002/erp/invoices/new');