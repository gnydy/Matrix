...
async function changeStock(tx: any, args: { companyId: string; productId: string; warehouseId: string; quantity: number; direction: 'in' | 'out'; refType: string; refId: string; note?: string; userId?: string }) {
  ...
  if (next < 0) {
    throw new Error(`رصيد المنتج ${product.sku} غير كافٍ. المتاح: ${current}`);
  }
...
async function ensureCashParty(tx: any, companyId: string) {
  let party = await tx.erpParty.findFirst({ where: { companyId, deletedAt: null, name: 'عميل نقدي', type: { in: ['customer', 'both'] } } });
  if (!party) {
    party = await tx.erpParty.create({ data: { companyId, type: 'customer', name: 'عميل نقدي' } });
  }
  return party;
}
...
function lineInput(rawLines: any[]) {
  return rawLines.map((line) => {
    ...
    const description = text(line.description) || 'بند';

    if (quantity <= 0) throw new Error('يوجد بند بكمية غير صحيحة');
    if (unitPrice < 0) throw new Error('يوجد بند بسعر غير صحيح');
...
app.post('/api/v1/erp/inventory/warehouses', asyncRoute(async (req: any, res: any) => {
  ...
  if (!name) throw new Error('اكتب اسم المخزن');
...
app.post('/api/v1/erp/inventory/products', asyncRoute(async (req: any, res: any) => {
  ...
  const name = text(req.body.name);
  if (!sku) throw new Error('اكتب كود المنتج');
...
app.post('/api/v1/erp/inventory/stock/adjust', asyncRoute(async (req: any, res: any) => {
  ...
  if (!productId || !warehouseId) throw new Error('اختر المنتج والمخزن');
...
app.delete('/api/v1/erp/sales/parties/:id', asyncRoute(async (req: any, res: any) => {
  ...
  if (docCount || recCount) throw new Error('لا يمكن حذف عميل عليه فواتير أو مديونيات');
...
app.post('/api/v1/erp/sales/documents', asyncRoute(async (req: any, res: any) => {
  ...
  if (!partyId) throw new Error('اختر العميل');
  if (!rawLines.length) throw new Error('أضف بنود الفاتورة');
...
app.patch('/api/v1/erp/sales/documents/:id/post', asyncRoute(async (req: any, res: any) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!doc) throw new Error('فاتورة البيع غير موجودة');
    if (doc.status === 'posted') return doc;

    for (const line of doc.lines) {
      if (line.productId) {
        await changeStock(tx, { companyId: req.companyId, productId: line.productId, warehouseId, quantity: toNumber(line.quantity), direction: 'out', refType: 'sales', refId: doc.id, note: `فاتورة بيع ${doc.ref}`, userId: req.user.id });
      }
    }
    ...
    await addJournal(tx, req.companyId, req.user.id, `فاتورة بيع ${doc.ref}`, [
      { accountCode: '1100', debit: toNumber(doc.grandTotal), memo: 'عميل / حسابات مدينة' },
      { accountCode: '4000', credit: toNumber(doc.grandTotal), memo: 'إيراد مبيعات' },
    ]);
    return posted;
  });
...
app.delete('/api/v1/erp/sales/documents/:id', asyncRoute(async (req: any, res: any) => {
  ...
  if (doc.status === 'posted') throw new Error('لا يمكن حذف فاتورة مرحلة');
...
app.post('/api/v1/erp/sales/cashier-sale', asyncRoute(async (req: any, res: any) => {
  ...
  const rawLines = Array.isArray(req.body.lines) ? req.body.lines : [];
  if (!warehouseId) throw new Error('اختر المخزن');
  if (!rawLines.length) throw new Error('لا توجد منتجات في الفاتورة');

  const result = await prisma.$transaction(async (tx) => {
    const party = await ensureCashParty(tx, req.companyId);
    const lines = lineInput(rawLines);
    const subtotal = lines.reduce((sum, line) => sum + line.lineTotal, 0);
    if (paidAmount < subtotal) throw new Error('المبلغ المدفوع أقل من إجمالي الفاتورة');

    const doc = await tx.erpSalesDocument.create({
      data: { companyId: req.companyId, partyId: party.id, type: 'invoice', ref: ref('POS'), note: 'بيع كاشير نقدي', subtotal, grandTotal: subtotal, status: 'posted', postedAt: new Date(), lines: { create: lines } },
      include: { party: true, lines: { include: { product: true } } },
    });

    for (const line of lines) {
      if (line.productId) {
        await changeStock(tx, { companyId: req.companyId, productId: line.productId, warehouseId, quantity: line.quantity, direction: 'out', refType: 'cashier_sale', refId: doc.id, note: `بيع كاشير ${doc.ref}`, userId: req.user.id });
      }
    }

    await addJournal(tx, req.companyId, req.user.id, `بيع كاشير نقدي ${doc.ref}`, [
      { accountCode: '1000', debit: subtotal, memo: 'نقدية من الكاشير' },
      { accountCode: '4000', credit: subtotal, memo: 'إيراد مبيعات' },
    ]);
...
app.delete('/api/v1/erp/purchases/parties/:id', asyncRoute(async (req: any, res: any) => {
  ...
  if (docCount || payCount) throw new Error('لا يمكن حذف مورد عليه فواتير أو مديونيات');
  const row = await prisma.erpParty.update({ where: { id }, data: { deletedAt: new Date() } });
  res.json(ok(row));
}));
...
app.post('/api/v1/erp/purchases/documents', asyncRoute(async (req: any, res: any) => {
  const partyId = text(req.body.partyId);
  const rawLines = Array.isArray(req.body.lines) ? req.body.lines : [];
  if (!partyId) throw new Error('اختر المورد');
  if (!rawLines.length) throw new Error('أضف بنود الفاتورة');
...
app.patch('/api/v1/erp/purchases/documents/:id/post', asyncRoute(async (req: any, res: any) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!doc) throw new Error('فاتورة الشراء غير موجودة');
    if (doc.status === 'posted') return doc;

    for (const line of doc.lines) {
      if (line.productId) {
        if (!warehouseId) throw new Error('اختر المخزن لإضافة المنتجات');
        await changeStock(tx, { companyId: req.companyId, productId: line.productId, warehouseId, quantity: toNumber(line.quantity), direction: 'in', refType: 'purchase', refId: doc.id, note: `فاتورة شراء ${doc.ref}`, userId: req.user.id });
      }
    }
    ...
    await addJournal(tx, req.companyId, req.user.id, `فاتورة شراء ${doc.ref}`, [
      { accountCode: '1200', debit: toNumber(doc.grandTotal), memo: 'مخزون / مشتريات' },
      { accountCode: '2000', credit: toNumber(doc.grandTotal), memo: 'مورد / حسابات دائنة' },
    ]);
    return posted;
  });
...
app.delete('/api/v1/erp/purchases/documents/:id', asyncRoute(async (req: any, res: any) => {
  ...
  if (!doc) throw new Error('فاتورة الشراء غير موجودة');
  if (doc.status === 'posted') throw new Error('لا يمكن حذف فاتورة شراء مرحلة');
...
app.post('/api/v1/erp/accounting/accounts', asyncRoute(async (req: any, res: any) => {
  ...
  const name = text(req.body.name);
  if (!code || !name) throw new Error('اكتب كود واسم الحساب');
...
app.post('/api/v1/erp/accounting/entries', asyncRoute(async (req: any, res: any) => {
  const rawLines = Array.isArray(req.body.lines) ? req.body.lines : [];
  if (rawLines.length < 2) throw new Error('القيد يجب أن يحتوي على سطرين على الأقل');
  ...
  if (Math.abs(debit - credit) > 0.001 || debit <= 0) throw new Error('القيد غير متزن');
...
app.post(['/api/v1/erp/hr/attendance', '/api/v1/erp/hr/attendances'], asyncRoute(async (req: any, res: any) => {
  ...
  if (!employeeId || !date) throw new Error('اختر الموظف والتاريخ');
...
app.post(['/api/v1/erp/hr/payroll', '/api/v1/erp/hr/payrolls', '/api/v1/erp/hr/payroll-runs'], asyncRoute(async (req: any, res: any) => {
  ...
  if (!employeeId || !month) throw new Error('اختر الموظف والشهر');
...
app.post('/api/v1/erp/projects', asyncRoute(async (req: any, res: any) => {
  const name = text(req.body.name);
  if (!name) throw new Error('اكتب اسم المشروع');
...
app.post('/api/v1/erp/manufacturing/orders', asyncRoute(async (req: any, res: any) => {
  ...
  if (!product) throw new Error('اكتب المنتج المطلوب تصنيعه');
...
app.use((error: any, _req: any, res: any, _next: any) => {
  console.error(error);
  res.status(400).json({ ok: false, error: error?.message || 'حدث خطأ غير متوقع' });
});
