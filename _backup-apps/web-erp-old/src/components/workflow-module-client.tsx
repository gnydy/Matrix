...
export function WorkflowModuleClient() {
  ...
  async function load() {
    ...
    try {
      const data = await erpApi<Row[]>('workflow/instances').catch(() => []);
  ...
  return (
    <div style={{ display: 'grid', gap: 24 }}>
      <section style={{ padding: 24, border: '1px solid #334155', borderRadius: 16, background: '#111827' }}>
        ...
        <p style={{ color: '#cbd5e1' }}>متابعة الموافقات والطلبات وخطوات المراجعة.</p>
      ...
      <section style={{ padding: 20, border: '1px solid #334155', borderRadius: 16, background: '#111827' }}>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          {!rows.length ? <p style={{ color: '#94a3b8' }}>لا توجد عمليات متابعة مسجلة.</p> : null}
        </div>
      </section>
    </div>
  );
