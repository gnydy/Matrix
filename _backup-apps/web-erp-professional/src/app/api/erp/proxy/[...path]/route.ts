...
function apiBase() {
  return (process.env.MATRIX_API_URL || 'http://127.0.0.1:4000').trim().replace(/\/$/, '');
}
...
async function proxy(request: Request, context: RouteContext) {
  ...
  const targetUrl = `${apiBase()}/api/v1/erp/${path.join('/')}${sourceUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: 'تعذر الاتصال بالـ API', targetUrl }, { status: 502 });
  }
}
